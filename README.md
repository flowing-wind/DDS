...
SPI slave port** (mode 0) and driving **one or two 14-bit parallel DACs**.
Written in plain Verilog-2001 — no vendor IP, no Block Memory Generator, no
`.coe` files; all memories are inferred BRAM.
...
- **Single clock domain**: the SPI pins are oversampled, so there is no CDC to
  constrain and no second bus clock
...
rtl/                portable Verilog-2001, no vendor primitives, simulatable
  dds_top.v         the IP core - instantiate this (SPI + 1..2 channels)
...
Doc/
  ...
  DDS_Tutorial_zh.md 中文入门教程: DDS 原理、每个功能、扫频/FSK、命令详解、上手步骤
...
9 MHz — the same waveform the STM32 firmware produces. Add `-DSCK_FAST` to rerun
the suite at the documented `dds_clk / 6` limit (16 MHz). A `tb_dds.vcd`
waveform dump is produced.

## Resource usage (7-series estimate)

Per channel: ~4 × BRAM18 (sine LUT + wave RAM), 1 × DSP48 (amplitude
multiplier), roughly 600 LUT / 900 FF. The shared SPI slave adds ~80 LUT /
120 FF. Comfortably exceeds 150 MHz on Artix-7 speed grade −1.
